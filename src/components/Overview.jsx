...
export default function Overview() {
  ...
  return (
    <div className="flex flex-col gap-10">
      <span className="flex items-center gap-5 stat-value text-emerald-400 bg-base-200 border border-base-300 rounded-xl p-2 w-fit">
        <FaWallet /> Total amount: ${totalAmount.toFixed(2)}
      </span>
      <span className="flex flex-col gap-3 text-emerald-400 bg-base-200 border border-base-300 rounded-xl p-2 w-fit">
        <FaArrowTrendUp /> Started: 12.02.2024 <br></br> Growth: ${totalChange().toFixed(2)} in{" "}
        {(data.length - 1) * 3} months ~ {((data.length - 1) * 3) / 12} years
      </span>
  ...
  return (
    <>
      {Object.entries(entry).map(([date, investments]) => (
        <div
          key={date}
          className="flex flex-col items-center gap-10 bg-base-200 border border-base-300 rounded-xl p-2"
        >
          <span className="flex items-center gap-2 text-xl font-bold ">
            <FaRegCalendarAlt /> {date}
          </span>
          <div className="flex gap-20 bg-base-100 border border-base-100 rounded-xl p-2 shadow">
            ...
            <span className="flex items-center gap-2">
              <SiCashapp />
              Total amount: {totalAmountPerQuartal(investments)}
            </span>
          </div>
        </div>
      ))}
